...
static int usage(void)
{
	...
	printf("   %s [--simulate] /dev/input/eventX\n", program_invocation_short_name);
	printf("     --simulate  simulate input event\n");
	printf("\n");
 ...
 * 
 */
static void send_event(int fd,struct input_event *ev)
{
 ...
 * @return 0 on success or 1 otherwise.
 */
static int simulate_events(int fd)
{
	struct input_event ev;
	int i,code,type,value;
	
	while (1) {
		printf("Please enter the events (format:type code value):");
		if( scanf("%d%d%d",&type,&code,&value) != 3 ) {
			printf(" Error input!!! Plase check it.\n");
			break;
		}
 ...
 * @return 0 on success, non-zero on error.
 */
static int do_simulate(const char *device)
{
	int fd;
	char *filename = NULL;
	...
	printf("Simulatinig ... (interrupt to exit)\n");

	free(filename);

	return simulate_events(fd);
...
static const struct option long_options[] = {
	...
	{ "simulate", no_argument, NULL, MODE_SIMULATE },
...
int main (int argc, char **argv)
{
	...
	if (mode == MODE_SIMULATE) {
		printf(" This option is simulation.\n");
		return do_simulate(device);
	}
